...
| `behavior.actions[].k8s.context.inCluster`      | Boolean indicating whether the Kubernetes configuration is within the cluster. |
| `behavior.actions[].k8s.context.configFilepath` | Path to the Kubernetes configuration file. |
| `behavior.actions[].k8s.context.masterUrl`      | URL of the Kubernetes master server. |
| `behavior.actions[].k8s.namespace`              | Kubernetes namespace where the pod is located. |
| `behavior.actions[].k8s.pod`                    | Kubernetes pod where the container is located. |
| `behavior.actions[].k8s.container`              | Kubernetes container where the command is executed. |
| `behavior.actions[].k8s.command`                | Kubernetes command to execute as part of the action. |
...
> **NOTE**
>
> With a simple source the original config is used in target configuration.
